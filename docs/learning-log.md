...
## 住宿基点路线最小版本（2026-09-27）

- 原因：先跑通少量输入到每日可查看路线的闭环，再通过实际使用优化质量，避免继续逐字段设计。
- 修改：三个独立上下文角色按阶段搜集、安排、修改；备注完整交给模型，无关键词或正则意图识别。地点按高德来源核实，程序计算交通、时间和已知费用；仍使用原任务、结果、Span 存储。前端提供简化输入、住宿基点、每日活动与交通、地图和问题定位；旧结果保留原始数据入口。
- 实际问题：地图搜索没有坐标时补查地点详情；真实公交查询曾长期阻塞，新增单次25秒可配置超时并退出MCP会话；模型将一般偏好当成必去地点，通过提示词与字段描述修正；地图标签改用经文本转义的HTML字符串。
- 验证：43项后端测试通过；前端类型检查与构建通过。浏览器实际北京一日游生成并保存，6次模型调用、41次地图调用（39成功、2超时），预算与排除地点被正确理解，公交未知耗时未计为零。替代依赖的两日样例验证日期切换、问题定位；旧格式入口通过。中断任务在重启后正确标记。验收数据使用临时SQLite，不进入Git。
- 限制：本次真实生成约6分钟；路线质量仍需优化（如重复面食、候选是否适合游览），公交超时只完成容错，未解决第三方响应问题。夜游替代、营业预约核实、住宿价格等完整目标保留在需求文档；地图虚线只是地点顺序。生产构建仍有主包体积提示。
